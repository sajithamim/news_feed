...
class FilteredListSerializertopic(serializers.ListSerializer):

    def to_representation(self, data):
        data = data.filter(title__icontains=self.context['key'])
        return super(FilteredListSerializertopic, self).to_representation(data)


class TopicSearchSerializer(serializers.ModelSerializer):
    class Meta:
        model = Topics
        list_serializer_class = FilteredListSerializertopic
        fields = ['id','title']
    # def to_representation(self, instance):
    #     response = super().to_representation(instance)
    #     response['category_id'] = CategorySerializer(instance.category_id).data
